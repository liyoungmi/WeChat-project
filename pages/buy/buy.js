...
Page({
  ...
  onShow: function () {
    ...
    // console.info("缓存数据：" + arr[0].title);    // 有数据的话，就遍历数据，计算总金额 和 总数量  
    if (arr.length > 0) {
      // 更新数据  
      this.setData({
        carts: arr,
  ...
  switchSelect: function (e) {
    ...
    for (i = 0; i < this.data.carts.length; i++) {
      Allprice = Allprice + (this.data.carts[index].money * this.data.carts[index].count);
    }
  ...
  allSelect: function (e) {
    ...
    if (!this.data.isAllSelect) {
      ...
      for (i = 0; i < this.data.carts.length; i++) {
        ...
        this.data.totalMoney = this.data.totalMoney + (this.data.carts[i].money * this.data.carts[i].count);
  ...
  //       var initID = res.data.toString();
  //       console.log(typeof initID)
  //       var init = Data.searchmtdata(initID)
  //       console.log(init);
  //       that.setData({
